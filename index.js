...
ws.on('connection', (socket) => {
	...
	socket.on('UserMessage', async message => {
		...
		let data = resp.data || {}
		message.pfp = data.user ? data.user.image : 'https://www.gravatar.com/avatar/70f68d9254a26e13edbd59e97869969b?d=https://repl.it/public/images/evalbot/evalbot_24.png&s=256' 
...
app.get('/', (req, res) => {
	...
	if(!whitelist.includes(username)) return res.send('not whitelisted L')
	res.render('index', {
		user: {
			name: username
		}
...
app.get('/lg', (req, res) => {
	res.send(`		<script authed="location.href = '/' " src="https://auth.util.repl.co/script.js"></script>`)
})
